...
//
// Author:	KEXIK
// Purpose:	ScrollBar (horizontal and vertical)
//-----------------------------------------------------------------------------
#pragma once
...
class CScrollBarVert : public IGuiElement
{
public:
	CScrollBarVert();
	void RenderLine(float deltaTime);
	bool CreateScrollBar(int x, int y, int w, int h, IGuiElement* pParent);
...
private:
	...
	//
	bool m_bChanged;
	int  m_bRelY; //je-li -1, nejde o posun, jinak je to rel pozice X od X posuvniku
	// GUI EDITOR
	int edit_nRelPosX;
	int edit_nRelPosY;
