...
//
// Author:	KEXIK
// Purpose:	Slider phys. constraint - entity class
//-----------------------------------------------------------------------------
#include "ePhys_slider.h"

REGISTER_ENTITY(ePhys_slider, "phys_slider", false, true, false);

ePhys_slider::ePhys_slider()
{
...
void ePhys_slider::Finalize()
{
	...
	if (!m_pBody1) // pokud je 1. NULL, musi byt nastaven jako child ten druhy
		g_pNewtonWorld->ConstraintCreateSlider(&m_pJoint, m_vPos, m_vDir, m_pBody2, m_pBody1, m_fMaxForce);
	else // je to naopak nebo jsou to 2 platne telesa
		g_pNewtonWorld->ConstraintCreateSlider(&m_pJoint, m_vPos, m_vDir, m_pBody1, m_pBody2, m_fMaxForce);

	if(!m_pJoint) Kill(this);
}

void ePhys_slider::Think(float deltaTime)
{
...
void ePhys_slider::Break(eBase* pCaller, int nValue)
{
