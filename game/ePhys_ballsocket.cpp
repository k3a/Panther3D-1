...
//
// Author:	KEXIK
// Purpose:	Ball and socket phys. constraint - entity class
//-----------------------------------------------------------------------------
#include "ePhys_ballsocket.h"

REGISTER_ENTITY(ePhys_ballsocket, "phys_ballsocket", false, true, false);

ePhys_ballsocket::ePhys_ballsocket()
{
...
void ePhys_ballsocket::Finalize()
{
	...
	if (!m_pBody1) // pokud je 1. NULL, musi byt nastaven jako child ten druhy
		g_pNewtonWorld->ConstraintCreateBall(&m_pJoint, m_vPos, m_pBody2, m_pBody1, m_fMaxForce);
	else // je to naopak nebo jsou to 2 platne telesa
		g_pNewtonWorld->ConstraintCreateBall(&m_pJoint, m_vPos, m_pBody1, m_pBody2, m_fMaxForce);

	if(!m_pJoint) Kill(this);
}

void ePhys_ballsocket::Think(float deltaTime)
{
...
void ePhys_ballsocket::Break(eBase* pCaller, int nValue)
{
